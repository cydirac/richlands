<?php
class ControllerThmblogComment extends Controller {

	private $error = array();

	public function index() { 
		$this->load->language('thmblog/comment');
		$this->document->setTitle($this->language->get('heading_title'));

		$this->load->model('thmblog/comment');
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) { 
			$this->model_thmblog_comment->editComment($this->request->get['comment_id'], $this->request->post);
			...
			$this->response->redirect($this->url->link('thmblog/comment', 'token=' . $this->session->data['token'] . $url, true));
		}
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $comment_id) {
				$this->model_thmblog_comment->deleteComment($comment_id);
			}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('thmblog/comment', 'token=' . $this->session->data['token'] . $url, true)
		);
		$data['add'] = $this->url->link('thmblog/comment/add', 'token=' . $this->session->data['token'] . $url, true);
		$data['delete'] = $this->url->link('thmblog/comment/delete', 'token=' . $this->session->data['token'] . $url, true);
		...
		$comment_total = $this->model_thmblog_comment->getTotalComments($filter_data);


		$results = $this->model_thmblog_comment->getArticleComments($filter_data);
		...
		foreach ($results as $result) {
			$data['articlecomments'][] = array(
				...
				'edit'       => $this->url->link('thmblog/comment/edit', 'token=' . $this->session->data['token'] . '&comment_id=' . $result['comment_id'] . $url, true)
		...
		$data['sort_articlename'] = $this->url->link('thmblog/comment', 'token=' . $this->session->data['token'] . '&sort=md.name' . $url, true);
		$data['sort_user'] = $this->url->link('thmblog/comment', 'token=' . $this->session->data['token'] . '&sort=mbc.user' . $url, true);
		$data['sort_status'] = $this->url->link('thmblog/comment', 'token=' . $this->session->data['token'] . '&sort=mbc.status' . $url, true);
		$data['sort_createdat'] = $this->url->link('thmblog/comment', 'token=' . $this->session->data['token'] . '&sort=mbc.created_at' . $url, true);
		...
		$pagination->url = $this->url->link('thmblog/comment', 'token=' . $this->session->data['token'] . $url . '&page={page}', true);
		...
		$this->response->setOutput($this->load->view('thmblog/comment', $data));
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['comment_id'])) {
			$data['action'] = $this->url->link('thmblog/comment/add', 'token=' . $this->session->data['token'] . $url, true);
		} else {
			$data['action'] = $this->url->link('thmblog/comment/edit', 'token=' . $this->session->data['token'] . '&comment_id=' . $this->request->get['comment_id'] . $url, true);
		}

		$data['cancel'] = $this->url->link('thmblog/comment', 'token=' . $this->session->data['token'] . $url, true);

		$this->load->model('thmblog/comment');

		if (isset($this->request->get['comment_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			$comment_info = $this->model_thmblog_comment->getBlogcomment($this->request->get['comment_id']);
		}
		...
		/*$data['parentcategory'] = $this->model_thmblog_comment->getBlogall(0);
		...
		$this->response->setOutput($this->load->view('thmblog/comment_form', $data));
	}

	public function add() { 
		$this->language->load('thmblog/comment');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) { 
			$this->model_thmblog_comment->addComment($this->request->post);		
			
			$this->session->data['success'] = $this->language->get('text_success');
						
			$this->response->redirect($this->url->link('thmblog/comment', 'token=' . $this->session->data['token'], true));
		}
	...
	protected function validateForm() {
		if (!$this->user->hasPermission('modify', 'thmblog/comment')) {
	...
	public function autocomplete() {
			...
			if (isset($this->request->get['article_name'])) {
				...
				$results = $this->model_thmblog_article->getArticles($data);
