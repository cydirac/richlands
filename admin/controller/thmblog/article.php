...
class ControllerThmblogArticle extends Controller {
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) { 
			$this->model_thmblog_article->editArticle($this->request->get['blog_id'], $this->request->post);
			...
			$this->response->redirect($this->url->link('thmblog/article', 'token=' . $this->session->data['token'] . $url, true));
		}
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $blog_id) {
				$this->model_thmblog_article->deleteArticle($blog_id);
			}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('thmblog/article', 'token=' . $this->session->data['token'] . $url, true)
		);
		
		$data['add'] = $this->url->link('thmblog/article/add', 'token=' . $this->session->data['token'] . $url, true);
		$data['delete'] = $this->url->link('thmblog/article/delete', 'token=' . $this->session->data['token'] . $url, true);
		...
		foreach ($results as $result) {
			...
			$data['articles'][] = array(
				...
				'edit'       => $this->url->link('thmblog/article/edit', 'token=' . $this->session->data['token'] . '&blog_id=' . $result['blog_id'] . $url, true)
...
//print_r($data['articles']);exit();
		...
		$data['sort_name'] = $this->url->link('thmblog/article', 'token=' . $this->session->data['token'] . '&sort=md.name' . $url, true);
		$data['sort_status'] = $this->url->link('thmblog/article', 'token=' . $this->session->data['token'] . '&sort=m.status' . $url, true);
		...
		$pagination->url = $this->url->link('thmblog/article', 'token=' . $this->session->data['token'] . $url . '&page={page}', true);
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['blog_id'])) {
			$data['action'] = $this->url->link('thmblog/article/add', 'token=' . $this->session->data['token'] . $url, true);
		} else {
			$data['action'] = $this->url->link('thmblog/article/edit', 'token=' . $this->session->data['token'] . '&blog_id=' . $this->request->get['blog_id'] . $url, true);
		}

		$data['cancel'] = $this->url->link('thmblog/article', 'token=' . $this->session->data['token'] . $url, true);

		$this->load->model('thmblog/article');

		if (isset($this->request->get['blog_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			$article_info = $this->model_thmblog_article->getBlogarticle($this->request->get['blog_id']);
			$article_keyword_info = $this->model_thmblog_article->getBlogarticlekeyword($this->request->get['blog_id']);
		}
		...
		if (isset($this->request->post['article_description'])) {
			...
			$data['article_description'] = $this->model_thmblog_article->getarticleDescriptions($this->request->get['blog_id']);
		} else {
		...
		if (isset($this->request->post['article_category'])) { 
			...
			$data['article_category'] = $this->model_thmblog_article->getBlogarticlecategory($this->request->get['blog_id']);
		...
		if (isset($this->request->post['article_store'])) { 
			...
			$data['article_store'] = $this->model_thmblog_article->getBlogarticleStores($this->request->get['blog_id']);
		...
		if (isset($this->request->post['related_product'])) {
			...
			$products = $this->model_thmblog_article->getRelatedProduct($this->request->get['blog_id']);
...
// related articles
		if (isset($this->request->post['related_article'])) {
			...
			$related_article = $this->model_thmblog_article->getRelatedArticle($this->request->get['blog_id']);
		...
		$related_article_info = $this->model_thmblog_article->getarticleDescriptions($article_id);
		...
		$this->response->setOutput($this->load->view('thmblog/article_form', $data));
	}
	public function add() { 
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) { 
			$this->model_thmblog_article->addArticle($this->request->post);		
			
			$this->session->data['success'] = $this->language->get('text_success');
						
			$this->response->redirect($this->url->link('thmblog/article', 'token=' . $this->session->data['token'], true));
		}
	...
	protected function validateForm() {
		if (!$this->user->hasPermission('modify', 'thmblog/article')) {
		...
		if (utf8_strlen($this->request->post['keyword']) > 0) {
			...
			if ($url_alias_info && isset($this->request->get['blog_id']) && $url_alias_info['query'] != 'tmblogarticle_id=' . $this->request->get['blog_id']) {
