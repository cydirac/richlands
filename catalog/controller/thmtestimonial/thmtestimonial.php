<?php
class ControllerThmtestimonialThmtestimonial extends Controller {
	private $error = array();
	...
	public function index() {
		...
		if($isTestimonialStore){ 
	        ...
	        $data['breadcrumbs'][] = array(
	            ...
	            'href' => $this->url->link('thmtestimonial/thmtestimonial')
	        );
	        ...
	        $this->response->setOutput($this->load->view('thmtestimonial/thmtestimonial', $data));
	...
	public function testimonial() {
		...
		$data['thmtestimonials'] = array();

		$review_total = $this->model_thmtestimonial_thmtestimonial->getTotalTestimonial();
		if($data['items_limit']) { $limit=$data['items_limit'];} else { $limit=10; }
		$results = $this->model_thmtestimonial_thmtestimonial->getTestinomials(($page - 1) * $limit, $limit);
		...
		$pagination->url = $this->url->link('thmtestimonial/thmtestimonial/testimonial', 'page={page}');
		...
		$data['results'] = sprintf($this->language->get('text_pagination'), ($review_total) ? (($page - 1) * $limit) + 1 : 0, ((($page - 1) * $limit) > ($review_total - $limit)) ? $review_total : ((($page - 1) * $limit) + $limit), $review_total, ceil($review_total / $limit));
			...
			$this->response->setOutput($this->load->view('thmtestimonial/thmtestimonial_list', $data));
	...
	if($testimonial_settings['captcha_status']){ 		
			...
			if (!isset($json['error'])) {
				$this->load->model('thmtestimonial/thmtestimonial');
				$this->model_thmtestimonial_thmtestimonial->addTestimonial($this->request->post);
		...
		if( is_dir(DIR_IMAGE.'catalog/tmtestimonial/') === false ){ mkdir(DIR_IMAGE.'catalog/tmtestimonial/');}
		...
		if (!$json) {
			...
			move_uploaded_file($this->request->files['file']['tmp_name'], DIR_IMAGE.'catalog/tmtestimonial/' . $file);
