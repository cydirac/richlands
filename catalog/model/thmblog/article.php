...
class ModelThmblogArticle extends Model {
	 ...
	 public function getTotalArticles($data = array()){
		$sql = "SELECT COUNT(mb.blog_id) as total FROM " . DB_PREFIX . "thmblog mb
					   LEFT JOIN " . DB_PREFIX . "thmblog_description mbd ON (mb.blog_id = mbd.blog_id) 
                                           LEFT JOIN " . DB_PREFIX . "thmblog_to_store mb2s ON (mb.blog_id = mb2s.blog_id) 
	...
	public function getArticles($data = array()){
		$sql = "SELECT * FROM " . DB_PREFIX . "thmblog mb
					   LEFT JOIN " . DB_PREFIX . "thmblog_description mbd ON (mb.blog_id = mbd.blog_id) 
                                           LEFT JOIN " . DB_PREFIX . "thmblog_to_store mb2s ON (mb.blog_id = mb2s.blog_id) 
                                           WHERE mbd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND mb2s.store_id = '" . (int)$this->config->get('config_store_id') . "' AND mb.status = '1'";
	...
	public function addComment($tmblogarticle_id, $data) {		
		...
		$this->db->query("INSERT INTO " . DB_PREFIX . "thmblog_comment SET user = '" . $this->db->escape($data['name']) . "', blog_id = '" . (int)$tmblogarticle_id. "',  comment = '" . $this->db->escape($data['text']) . "', rating = '" . (int)$data['rating'] . "', created_at = NOW()");
	...
	public function getCommentsByArticleId($tmblogarticle_id, $start = 0, $limit = 20) {
		...
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "thmblog_comment mbc LEFT JOIN " . DB_PREFIX . "thmblog mb ON (mbc.blog_id = mb.blog_id) LEFT JOIN " . DB_PREFIX . "thmblog_description mbd ON (mb.blog_id = mbd.blog_id) 
	...
	public function getTotalCommentsByArticleId($tmblogarticle_id) {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "thmblog_comment mbc LEFT JOIN " . DB_PREFIX . "thmblog mb ON (mbc.blog_id = mb.blog_id) LEFT JOIN " . DB_PREFIX . "thmblog_description mbd ON (mb.blog_id = mbd.blog_id) 
					   WHERE mb.blog_id = '" . (int)$tmblogarticle_id . "' AND mbc.status = '1' 
					   AND mbd.language_id = '" . (int)$this->config->get('config_language_id') . "'");
	...
	public function getcategoryNamebyArticle($tmblogarticle_id){ 
	   $sql="SELECT mbd.name as categoryname,mbc.category_id as categoryid FROM " . DB_PREFIX . "thmblog_to_category mbc  
					 LEFT JOIN " . DB_PREFIX . "thmblogcategory_description mbd ON (mbc.category_id = mbd.category_id) 
					   WHERE mbc.blog_id = '" . (int)$tmblogarticle_id . "' GROUP BY mbc.category_id";
	     ...
	     $result='';
		  ...
		  foreach ($query->rows as $row) {
			  ...
			  $link=$this->url->link('thmblog/category', 'tmblogcategory_id=' . $categoryid);
