<?php
class ModelThmtestimonialThmtestimonial extends Model {
...
//		$this->event->trigger('post.testimonial.add', $testimonial_id);
	...
	public function getTestimonialSettings(){
	 ...
	 /* public function resize($filename, $width, $height) {
		...
		if (!is_file(DIR_IMAGE.'tmtestimonial/' . $filename)) {
			return;
		}
		...
		$old_image = $filename;
		$new_image = 'cache/catalog/tmtestimonial' . utf8_substr($filename, 0, utf8_strrpos($filename, '.')) . '-' . $width . 'x' . $height . '.' . $extension;

		if (!is_file(DIR_IMAGE. $new_image) || (filectime(DIR_IMAGE.'catalog/tmtestimonial/' . $old_image) > filectime(DIR_IMAGE.$new_image))) {
			$path = '';
...
print_r($directories);exit();
			...
			list($width_orig, $height_orig) = getimagesize(DIR_IMAGE.'tmtestimonial/' . $old_image);

			if ($width_orig != $width || $height_orig != $height) {
				$image = new Image(DIR_IMAGE.'tmtestimonial/' . $old_image);
				$image->resize($width, $height);
				$image->save(DIR_IMAGE. $new_image);
			} else {
				copy(DIR_IMAGE.'catalog/tmtestimonial/' . $old_image, DIR_IMAGE . $new_image);
			}
		}
		...
		public function resize($filename, $width, $height) {
			$filename = 'catalog/tmtestimonial/'.$filename;
